...
app.get('/health', (req, res) => {
    res.json({ success: true, message: 'Notification service is healthy', data: { timestamp: new Date().toISOString(), uptime: process.uptime() } });
});
...
const startServer = async () => {
    try {
        ...
        app.listen(PORT, () => { logger_1.logger.info(`🔔 Notification Service running on port ${PORT}`); });
    }
    catch (error) {
