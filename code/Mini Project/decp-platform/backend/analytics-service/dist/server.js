...
app.get('/health', (req, res) => {
    res.json({ success: true, message: 'Analytics service is healthy', data: { timestamp: new Date().toISOString(), uptime: process.uptime() } });
});
app.use('/', analyticsRoutes_1.default);
...
const startServer = async () => {
    try {
        ...
        app.listen(PORT, () => { logger_1.logger.info(`📊 Analytics Service running on port ${PORT}`); });
    }
    catch (error) {
