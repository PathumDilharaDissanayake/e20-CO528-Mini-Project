...
app.get('/health', (req, res) => {
    res.json({ success: true, message: 'Events service is healthy', data: { timestamp: new Date().toISOString(), uptime: process.uptime() } });
});
...
const startServer = async () => {
    try {
        ...
        app.listen(PORT, () => { logger_1.logger.info(`📅 Events Service running on port ${PORT}`); });
    }
    catch (error) {
